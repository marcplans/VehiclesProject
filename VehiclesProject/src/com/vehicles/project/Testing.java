...
public class Testing {
	...
	public static void main(String[] args) {
		...
		String subStr = "fOOd";
		System.out
				.println(mainStr.toLowerCase().contains(subStr.toLowerCase()));
	...
	public static String userInput(Scanner userInput, String vehicleType,
		...
		System.out.println(HelpersMain.promptMessage(vehicleType, carMessage,
				bikeMessage));
		String userInputStr;
		while ((userInputStr = userInput.nextLine()).isEmpty()) {
			System.out.println("Not valid, input can not be empty!");
			System.out.println(HelpersMain.promptMessage(vehicleType,
					carMessage, bikeMessage));
		}
		return userInputStr;
	}

	public static boolean isValidDiameter(String diameter) {
		return Double.parseDouble(diameter) > 0.4 && isNumeric(diameter)
				&& Double.parseDouble(diameter) < 4;
	}

	// check if string content are digits so that later can be converted to
	// double type
 ...
 * 
 * // Checks if string is numeric, if so returns not empty numeric input string
 * } else if (isNumeric(inputStr)) { return inputStr;
 * 
 * // Input string is not empty and not numeric, but numeric string is required
 * } else { System.out.println( "Not valid, numeric string is required!"); } } }
 * 
 * 
 * // Checks if user input is empty, if so, asks for input again public static
 * String userInput(Scanner userInput, String promptMessage) { return
...
/*
 * // Checks if user input matches expected value. If not, prompts error and //
 * asks for new input public static String userInput(Scanner userInput, String
 * promptMessage, boolean checkPlate, boolean checkDiameter) {
 * 
 * while (true) { System.out.println(promptMessage); String inputStr =
 * userInput.nextLine();
 * 
 * // Input string is empty, so jump back to loop start if (inputStr.isEmpty())
 * { System.out.println("Not valid, input is empty!"); continue;
 * 
 * // Input string is not empty and check plate and diameter // are not
 * required, so base case input string is returned } else if (!checkPlate &&
 * !checkDiameter) { return inputStr;
 * 
 * // Input string is not empty, check plate is required and string // matches
 * expected plate format, so plate imput string is // returned } else if
 * (checkPlate && isValidPlate(inputStr)) { return inputStr;
 * 
 * // Input string is not empty, check plate is required and string // does not
 * // match expected plate format, prompts error and jumps to loop // start }
 * else if (checkPlate && !isValidPlate(inputStr)) { System.out.println(
 * "Not valid, plate must be 4 numbers followed by 2 or 3 letters!"); continue;
 * 
 * // Input string is not empty, check diameter is required and // string // is
 * a valid diameter value, so diameter input string is // returned } else if
 * (checkDiameter && isValidDiameter(inputStr)) { return inputStr;
 * 
 * // Input string is not empty, check diameter is required and // string // is
 * not a valid diameter value, prompts error and jumps to // loop start } else
 * if (checkDiameter && !isValidDiameter(inputStr)) { System.out.println(
 * "Not valid, diameter must be a numeric value > 0.4 and < 4"); } } }
 * 
 * // Checks if user input is empty, if so, asks for input again public static
 * String userInput(Scanner userInput, String promptMessage) { return
 * userInput(userInput, promptMessage, false, false); }
 * 
 * // Checks if user input is empty, and takes checkOption boolean to check //
 * plate or diameter // If checkOption is true checks plate format, if it is
 * false checks wheel // diameter string public static String userInput(Scanner
 * userInput, String promptMessage, boolean checkOption) { return
 * userInput(userInput, promptMessage, checkOption, !checkOption); }
 * 
 */
>>>>>>> refs/heads/branch1
